        ...
        <form method="POST" action="{{ $plan ? route('admin.plans.update', $plan) : route('admin.plans.store') }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="flex flex-col gap-3 justify-end">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_private" value="1" {{ old('is_private', $plan->is_private ?? false) ? 'checked' : '' }}
                               class="w-4 h-4 accent-indigo-600">
                        <span class="text-sm font-semibold text-indigo-700">Private Custom Offer (hidden from public)</span>
                    </label>
                </div>
            </div>
