...
<section class="py-8 lg:py-12" id="admin-users">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex gap-2 mb-6 flex-wrap">
            <a href="{{ route('admin.users') }}" class="px-4 py-2 text-sm font-medium rounded-sm transition-all {{ !request('role') ? 'bg-[#2563EB]/10 text-[#2563EB] border border-[#2563EB]/50' : 'bg-stone-50 text-zinc-500 border border-stone-200/50 hover:bg-stone-100' }}" title="All">
                All
            </a>
            <a href="{{ route('admin.users', ['role' => 'owner']) }}" class="px-4 py-2 text-sm font-medium rounded-sm transition-all {{ request('role') === 'owner' ? 'bg-indigo-600/20 text-indigo-500 border border-indigo-600/30' : 'bg-stone-50 text-zinc-500 border border-stone-200/50 hover:bg-stone-100' }}" title="Owners">
                Owners
            </a>
            <a href="{{ route('admin.users', ['role' => 'tenant']) }}" class="px-4 py-2 text-sm font-medium rounded-sm transition-all {{ request('role') === 'tenant' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-stone-50 text-zinc-500 border border-stone-200/50 hover:bg-stone-100' }}" title="Tenants">
                Tenants
            </a>
            <a href="{{ route('admin.users', ['role' => 'admin']) }}" class="px-4 py-2 text-sm font-medium rounded-sm transition-all {{ request('role') === 'admin' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-stone-50 text-zinc-500 border border-stone-200/50 hover:bg-stone-100' }}" title="Admins">
                Admins
            </a>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-stone-100 transition-colors cursor-pointer" onclick="window.location.href='{{ route('admin.users.activity', $user) }}'" title="View User Profile & Activity">
                            <td class="px-6 py-4">
