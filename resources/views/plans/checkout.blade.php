...
<section class="checkout-stage min-h-screen py-6 sm:py-10 w-full" id="checkout-page">
    <div class="mx-auto w-full max-w-5xl px-3 sm:px-6 lg:px-8">
        ...
        <div class="mb-4 px-1 flex items-center justify-between">
            <a href="{{ route('plans.index') }}" class="inline-flex items-center gap-2 text-xs sm:text-sm font-semibold text-slate-500 transition hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400" title="Back to plans">
            ...
            <span class="inline-flex items-center gap-1.5 text-xs font-medium text-slate-400 dark:text-slate-500">
                <i class="ph-bold ph-lock-key text-emerald-500 text-sm"></i>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6 items-stretch">
            ...
            <div class="lg:col-span-5 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 rounded-3xl p-5 sm:p-6 text-white flex flex-col justify-between relative overflow-hidden shadow-lg shadow-blue-600/10 border border-blue-500/30">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex items-center justify-between gap-3 mb-4">
                        <div class="flex items-center gap-3">
                            <span class="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white/20 backdrop-blur-md text-white shadow-inner">
                                <i class="ph-bold ph-crown text-xl"></i>
                            </span>
                            <div>
                                <span class="text-[11px] font-semibold uppercase tracking-wider text-blue-200 block">Selected Plan</span>
                                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-white leading-tight">{{ $plan->name }}</h1>
                            </div>
                        </div>
                        <span class="px-3 py-1 bg-white/15 backdrop-blur-sm text-xs font-semibold rounded-full text-blue-100 border border-white/20 shrink-0">
                    ...
                    <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/15 mb-5 flex items-center justify-between">
                        <div>
                            <span class="text-[11px] font-semibold uppercase tracking-wider text-blue-200 block mb-0.5">Total Payable</span>
                            <span class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight leading-none">
                                ₹{{ number_format($billing['final'], 2) }}
                            </span>
                        </div>
                        <span class="px-3 py-1 bg-emerald-500/20 text-emerald-300 text-xs font-semibold rounded-full border border-emerald-400/30">
                            {{ $plan->purpose === 'buy' || $billingPeriod === 'yearly' ? 'Annual Pass' : 'Rental Pass' }}
                        </span>
                    </div>

                    {{-- Compact Plan Highlights list --}}
                    <div class="space-y-2.5 text-xs sm:text-[13px] font-normal text-blue-50/95">
                        <div class="flex items-center gap-2.5">
                            <i class="ph-bold ph-check-circle text-emerald-300 text-base shrink-0"></i>
                            <span><strong class="font-semibold text-white">{{ $plan->contact_limit }}</strong> Direct Owner Contact Unlocks</span>
                        </div>
                        <div class="flex items-center gap-2.5">
                            <i class="ph-bold ph-check-circle text-emerald-300 text-base shrink-0"></i>
            ...
            <div class="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xs flex flex-col justify-between space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    ...
                    {{-- Mobile Number Input --}}
                    <div class="rounded-2xl border border-slate-200/90 bg-slate-50/70 p-3.5 dark:border-slate-800 dark:bg-slate-850 flex flex-col justify-between">
                        <div class="flex items-center justify-between mb-1.5">
                            <label for="checkout_user_phone" class="text-[11px] font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300">
                                Mobile No <span class="text-red-500">*</span>
                            </label>
                            @if(!empty($userCleanPhone))
                                <span id="phone_sync_badge" class="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-600 dark:text-emerald-400">
                                    <i class="ph-bold ph-check-circle"></i> Synced
                                </span>
                            @else
                                <span id="phone_sync_badge" class="inline-flex items-center gap-1 text-[11px] font-medium text-blue-600 dark:text-blue-400">
                                    Auto-saves
                                </span>
                            @endif
                        </div>
                        
                        <div class="relative flex items-center bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 px-3 py-1.5 focus-within:border-blue-600 focus-within:ring-2 focus-within:ring-blue-500/15 transition-all">
                            <span class="text-xs font-bold text-slate-700 dark:text-slate-300 select-none flex items-center gap-1">
                                <span>🇮🇳 +91</span>
                                <span class="h-4 w-px bg-slate-200 dark:bg-slate-700 ml-1.5 mr-1"></span>
                            </span>
                            <input
                                type="tel"
                                ...
                                class="w-full bg-transparent text-xs sm:text-sm font-semibold text-slate-900 placeholder:text-slate-400 placeholder:font-normal outline-none dark:text-white"
                                autocomplete="tel-national"
                                inputmode="numeric"
                            >
                            <span id="phone_valid_icon" class="text-emerald-500 text-sm transition-opacity duration-200 ml-1 {{ !empty($userCleanPhone) ? 'opacity-100' : 'opacity-0' }}">
                                <i class="ph-bold ph-check-circle"></i>
                            </span>
                        </div>
                    </div>
                </div>
                <p id="phone_error_text" class="hidden text-xs font-semibold text-red-600 -mt-2">
                ...
                <div class="rounded-2xl border border-slate-200/80 bg-slate-50/60 p-3 dark:border-slate-800 dark:bg-slate-850 flex items-center justify-between flex-wrap gap-2">
                    ...
                    <div class="flex items-center gap-2 text-xs font-medium text-slate-700 dark:text-slate-300 flex-wrap">
                        <span class="px-2.5 py-1 bg-white dark:bg-slate-800 rounded-lg border border-slate-200/80 dark:border-slate-700 inline-flex items-center gap-1 shadow-2xs">
