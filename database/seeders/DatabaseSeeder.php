...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Create Owners
        $owner1 = User::where('email', 'owner1@unlockrentals.com')->first();
        if (!$owner1) {
            $owner1 = User::create([
                'name' => 'John Owner',
        ...
        $owner2 = User::where('email', 'owner2@unlockrentals.com')->first();
        if (!$owner2) {
            $owner2 = User::create([
                'name' => 'Sarah Landlord',
        ...
        // Create Tenants
        if (!User::where('email', 'tenant@unlockrentals.com')->exists()) {
            User::create([
                'name' => 'Michael Tenant',
        ...
        foreach ($categories as $cat) {
            $existingCat = Category::where('name', $cat['name'])->first();
            if (!$existingCat) {
                $existingCat = Category::create([
                    'name' => $cat['name'],
